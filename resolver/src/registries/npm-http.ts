...
export default class NpmHttpRegistry {
  ...
  fetch(name: string, cb: (err: boolean, result?: any) => void): void {
    ...
    if (this.cache[name]) {
      ...
      request.get(`${this.registryUrl}/${escapedName}`).end((err: any, res: any) => {
  ...
  batchFetch(keys: string[], cb: () => void): void {
    ...
    if (fetchKeys.length) {
      ...
      async.parallel(fetchKeys.map((key: string) => {
        const escapedName = key && npa(key).escapedName;
        return (done: () => void) => request.get(`${this.registryUrl}/${escapedName}`).end((err: any, res: any) => {
          if (err || res.statusCode < 200 || res.statusCode >= 400) { return done(); }
