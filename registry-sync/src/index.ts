...
const REGISTRY_RETRY_DELAY = parseInt(
  process.env.REGISTRY_RETRY_DELAY || '5000',
  10,
);
const REGISTRY_TIMEOUT_MINUTES = parseInt(
  process.env.REGISTRY_TIMEOUT_MINUTES || '10',
  10,
);
const STALE_RETRY_INTERVAL_MINUTES = parseInt(
  process.env.STALE_RETRY_INTERVAL_MINUTES || '6',
  10,
);
const CAUGHT_UP_DELAY_MS = parseInt(
  process.env.CAUGHT_UP_DELAY_MS || '7500',
  10,
);
const STALE_PACKAGE_HOURS = parseInt(
  process.env.STALE_PACKAGE_HOURS || '1',
  10,
);

function getLastSeq(): Promise<string> {
  return redis.get(SEQ_KEY);
}
function setLastSeq(seq: string): Promise<any> {
  return redis.set(SEQ_KEY, seq);
}
function _getPackage(name: string): Promise<any> {
  return redis.get(PKG_KEY_PREFIX + name).then((pkg) => JSON.parse(pkg));
}
function setPackage(name: string, value: any): Promise<any> {
  return redis.set(PKG_KEY_PREFIX + name, JSON.stringify(value));
}
function _deletePackage(name: string): Promise<any> {
  return redis.del(PKG_KEY_PREFIX + name);
}
function enqueueStaleRetry(name: string): Promise<any> {
  return redis.sadd(STALE_SET_KEY, name);
}
function dequeueStaleRetry(name: string): Promise<any> {
...
function getRegPackage(name: string): Promise<any> {
  ...
  return request
    .get({ url: `${REGISTRY_URL}/${escapedName}`, json: true })
    .catch((error: any) => {
      if (error && error.statusCode === 503) {
        console.warn(`503 Registry response fetching ${name}, retrying in 5s`);
        return new Promise((resolve, reject) =>
          setTimeout(() => resolve(getRegPackage(name)), REGISTRY_RETRY_DELAY),
        );
      }
      throw error;
    });
...
async function retryStalePackages(): Promise<void> {
  ...
  await Promise.all(
    (pkgs as string[]).map((pkgName: string) => {
...
function getLastRegSeq(): Promise<number> {
  return request
    .get({ url: `${COUCH_URL}?descending=true&limit=1`, json: true })
    .then((r: any) => r.last_seq);
}
...
function touchTimeout(): void {
  lastTouch = new Date();
}
function checkTimeout(): void {
  if (
    new Date().getTime() - lastTouch.getTime() >
    REGISTRY_TIMEOUT_MINUTES * MINUTE_MS
  ) {
    console.warn(
      `Haven't received data from registry in ${REGISTRY_TIMEOUT_MINUTES} minutes, exiting`,
    );
    process.exit(1);
  }
}
...
Promise.all([getLastSeq(), getLastRegSeq()]).then(([since, bootRegSeq]) => {
  ...
  follower(function (change: any, done: () => void) {
    ...
    const promise = !caughtUp
      ? Promise.resolve()
      : new Promise((resolve: any, reject: any) =>
          setTimeout(resolve, CAUGHT_UP_DELAY_MS),
        );

    promise.then(
      () =>
        getRegPackage(change.id)
          .then((pkg: any) => {
            console.info('ADDING:', change.id);
            if (caughtUp) {
              const pkgMod = new Date(pkg.time.modified);
              if (
                new Date().getTime() - pkgMod.getTime() >
                STALE_PACKAGE_HOURS * HOUR_MS
              ) {
                console.warn(
                  `WARNING: Registry data for ${change.id} may be stale, will retry later.`,
                );
