...
export const JSDELIVR_URL = process.env.CDN_BASE_URL || 'https://cdn.jsdelivr.net/npm';
export const RESOLVED_EXTENSIONS = ['.js', '.json', '/index.js', '/index.json'];

const REQUEST_TIMEOUT = parseInt(process.env.CDN_REQUEST_TIMEOUT || '60000', 10);
...
export async function fetchUrl(url: string, buffer = false): Promise<string | object> {
  return request.get(url).timeout(REQUEST_TIMEOUT).buffer(!!buffer).catch((error: any) => {
...
export async function fetchDirList(packageSlug: string): Promise<string[]> {
  const res = await fetchUrl(`https://data.jsdelivr.com/v1/package/npm/${packageSlug}/flat`);
...
async function fetchChild(
  ...
  if (['.js', '.json'].some(ext => _.endsWith(path, ext)) && (index = fileList.indexOf(`/${path}`)) !== -1) {
    ...
    if (i !== -1) { resPath = potPath; index = i; return true; }
    return false;
  })) {
    return;
  }

  if (!found && resPath === '') { return; }
